...
// Maps occasion/theme keys to email-safe gradient pairs (start color, end color)
const themeColors = {
...
function buildCardEmail({ recipientName, senderName, cardText, occasion }) {
  ...
  // Split card text into paragraphs for better rendering
...
<head>
  ...
  <title>${occasionLabel} from ${senderName || 'Someone Special'}</title>
</head>
    ...
    <tr>
      <td style="padding: 40px 20px;">
        ...
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 560px; margin: 0 auto;">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, ${theme.from} 0%, ${theme.to} 100%); border-radius: 16px 16px 0 0; padding: 40px 48px 32px; text-align: center;">
              <p style="margin: 0 0 8px 0; font-family: Georgia, serif; font-size: 13px; letter-spacing: 3px; text-transform: uppercase; color: rgba(255,255,255,0.85);">HeartPenned</p>
              <h1 style="margin: 0; font-family: Georgia, serif; font-size: 28px; font-weight: normal; color: #ffffff; letter-spacing: 1px;">${occasionLabel}</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color: #ffffff; padding: 48px 48px 40px; border-left: 1px solid #e8e8e8; border-right: 1px solid #e8e8e8;">
              ...
              ${greeting ? `
              <!-- Greeting -->
              <p style="margin: 0 0 24px 0; font-family: Georgia, serif; font-size: 18px; color: ${theme.accent}; font-style: italic;">${greeting}</p>
              ` : ''}

              <!-- Card text -->
              <div style="font-family: Georgia, serif; font-size: 17px; line-height: 1.8; color: #2d2d2d;">
                ${paragraphs}
              </div>

              ${signoff ? `
              <!-- Sign-off -->
              <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid ${theme.light}; font-family: Georgia, serif; font-size: 16px; color: #4a4a4a; font-style: italic;">
                ${signoff}
              </div>
              ` : ''}
          ...
          <tr>
            <td style="background: linear-gradient(135deg, ${theme.from} 0%, ${theme.to} 100%); height: 6px; border-radius: 0 0 16px 16px;"></td>
          </tr>

          <!-- Footer -->
          <tr>
            <td style="padding: 24px 0 0; text-align: center;">
              <p style="margin: 0; font-family: Arial, sans-serif; font-size: 12px; color: #999999; letter-spacing: 1px;">
                Sent with <a href="https://heartpenned.com" style="color: ${theme.accent}; text-decoration: none;">HeartPenned</a> · Because some words matter
              </p>
            </td>
          </tr>
...
export default async function handler(req, res) {
  ...
  try {
    const data = await resend.emails.send({
      ...
      to: [recipientEmail],
      subject,
      html: buildCardEmail({ recipientName, senderName, cardText, occasion }),
    });
